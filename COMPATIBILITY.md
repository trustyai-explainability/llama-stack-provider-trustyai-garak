...
|------------------|---------------|--------|------------------|-------|
| 0.5.1 | ==0.17.0+rhaiv.3 | >=3.12 | kfp>=2.14.6, eval-hub-sdk[adapter]~=0.4.2, boto3>=1.35.88 | Standard scans use `run.spec`. Legacy selection fields remain accepted. Garak does not replace `cas.expand_intent_tree` or `cas.trust_code_stubs`. |
